import { L as LitElement, p as pbMixin, r as resolveURL, h as html, c as css } from './pb-mixin-ae9e2885.js';
...
class PbMapLayer extends LitElement {
  ...
  async data() {
    return new Promise(resolve => {
      fetch(this.url).then(response => response.json()).then(json => resolve(json));
    });
  }
...
class PbLeafletMap extends pbMixin(LitElement) {
  ...
  firstUpdated() {
    ...
    window.ESGlobalBridge.instance.load("leaflet", leafletPath).then(() => window.ESGlobalBridge.instance.load("plugin", pluginPath));
    window.addEventListener("es-bridge-plugin-loaded", this._initMap.bind(this), {
      once: true
    });
  }
  ...
  _configureLayers() {
    ...
    const layers = L.control.layers(null, null, {
      collapsed: false
    });
