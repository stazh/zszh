(:~
    ...
    ODD: /db/apps/zszh/resources/odd/zszh-norm.odd
 :)
xquery version "3.1";
            ...
            return
                    ...
                    case element(teiHeader) return
                        if ($parameters?header='context') then
                            let $params := 
                                map {
                                    "persons": let $register := doc("/db/apps/zszh-data/person/person.xml") let $entries:=     for $p in root(.)//persName[@ref]   group by $k := $p/@ref     return     id($p[1]/@ref , $register)  return  for $p in $entries   order by $p/persName ascending   return    $p,
                        ...
                        if (@ref) then
                            ...
                            let $params := 
                                map {
                                    ...
                                    "ref": @ref,
                                    "value": let $id := replace(@ref, ' ', '-') let $target:= id($id, doc("/db/apps/zszh-data/place/place.xml")) let $letter := substring($id, 1, 1) => upper-case() return     if ($target) then ( <a href="../places/all/{$target/placeName[@type="main"]/string()}?key={$id}" target="_blank">{$target/placeName[@type="main"]/string()}</a>, <span>{" " || $target/placeName[@type="add"]/string()}</span>) else (<a href="https://www.ssrq-sds-fds.ch/places-db-edit/views/view-place.xq?id={$id}" target="_blank">{$id}</a>)
                                }
